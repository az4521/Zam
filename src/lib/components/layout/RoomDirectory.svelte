...
<ModalDialog
    onClose={closeModal}
    labelledBy="room-directory-title"
    panelClass="relative bg-discord-background rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6 flex flex-col gap-4 max-h-[85vh]"
>
    ...
    <!-- Enter anywhere in the search row runs the search. The row is a plain
         layout wrapper, not a control, so it gets no role of its own; the
         ignore used to ride on the hand-rolled panel div that wrapped it. -->
